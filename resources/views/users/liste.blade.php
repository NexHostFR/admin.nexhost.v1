...
<main class="container mx-auto px-4">
    <div>
        <a href="/users/create" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3">
    ...
    <div class="overflow-x-auto mt-10">
        <table class="min-w-full border border-gray-200 bg-white rounded-lg shadow">
            <thead class="bg-gray-100">
                <tr>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Nom Prénom</th>
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-600">Rôle</th>
                </tr>
            </thead>
            <tbody>
                @if (isset($usersListe) && $usersListe->isNotEmpty())
                    @foreach ($usersListe as $user)
                        <tr class="cursor-pointer hover:bg-gray-100 transition" onclick="window.location='/users/{{ $user->id }}'">
